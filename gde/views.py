...
#  These views are responsible for handling different functionalities of your e-commerce website.
...
def register_dispatcher(request):
    ...
    return render(
        request,
        "register2.html",
        {"user_form": user_form, "dispatcher_form": dispatcher_form},
    )
...
def edit_store(request):
    if request.method == "POST":
        ...
        store_form = StoreEditForm2(
            instance=request.user.store, data=request.POST, files=request.FILES
        )
        if user_form.is_valid() and store_form.is_valid():
            user_form.save()
            store_form.save()
    ...
    return render(
        request, "edit_store.html", {"user_form": user_form, "store_form": store_form}
    )
...
def edit_dispatcher(request):
    if request.method == "POST":
        ...
        dispatcher_form = DispatcherEditForm2(
            instance=request.user.dispatcher, data=request.POST, files=request.FILES
        )
        if user_form.is_valid() and dispatcher_form.is_valid():
            user_form.save()
